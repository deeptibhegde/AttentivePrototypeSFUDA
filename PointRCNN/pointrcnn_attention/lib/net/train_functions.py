...
def model_joint_fn_decorator():
    ModelReturn = namedtuple("ModelReturn", ['loss', 'tb_dict', 'disp_dict','prototype'])
    MEAN_SIZE = torch.from_numpy(cfg.CLS_MEAN_SIZE[0]).cuda()

    def model_fn(model, data,prototype):
        if cfg.RPN.ENABLED:
            ...
            # input_data['rpn_cls_label'] = data['rpn_cls_label']
        else:
            input_data = {}
        ...
        ret_dict,prototype = model(input_data,prototype)
        ...
        return ModelReturn(loss, tb_dict, disp_dict,prototype)
    ...
    def get_rcnn_loss(model, ret_dict, tb_dict):
        ...
        sim_weights = ret_dict['sim_weights']
        ind = ret_dict['ind']
        ...
        if cfg.RCNN.LOSS_CLS == 'SigmoidFocalLoss':
            ...
            for i in range(len(sim_weights)):
                batch_loss_cls[ind[0][i]] *= sim_weights[i]
            # except:
            # import pdb; pdb.set_trace()
