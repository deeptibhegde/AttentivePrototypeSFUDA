...
class KittiRCNNDataset(KittiDataset):
    ...
    def preprocess_rpn_training_data(self):
        ...
        for idx in range(0, self.num_sample):
            sample_id = int(self.image_idx_list[idx])
            scores = kitti_utils.objs_to_scores(self.get_label(sample_id))
            obj_list = self.filtrate_objects(self.get_label(sample_id),scores)
            if len(obj_list) == 0:
    ...
    def filtrate_objects(self, obj_list,scores=[]):
        """
        ...
        valid_obj_list = []
        k = -1
        if not len(scores) or not self.split == "train" :
            for obj in obj_list:
            ...
            for obj in obj_list:
                k += 1
                if obj.cls_type not in type_whitelist:# or scores[k]<0.0:  # rm Van, 20180928
                    continue
                if self.mode == 'TRAIN' and cfg.PC_REDUCE_BY_RANGE and (self.check_pc_range(obj.pos) is False):
                    continue
                valid_obj_list.append(obj)
