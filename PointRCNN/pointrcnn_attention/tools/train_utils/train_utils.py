...
class Trainer(object):
    ...
    def _train_it(self, batch, prototype):
        self.model.train()

        self.optimizer.zero_grad()
        loss, tb_dict, disp_dict, prototype = self.model_fn(self.model, batch,prototype)

        loss.backward()
        clip_grad_norm_(self.model.parameters(), self.grad_norm_clip)
        self.optimizer.step()

        return loss.item(), tb_dict, disp_dict, prototype
    ...
    def train(self, start_it, start_epoch, n_epochs, train_loader, test_loader=None, ckpt_save_interval=5,
        ...
        if start_epoch >0:
            ...
            prototype_path = os.path.join(self.ckpt_dir, 'proto/epoch_%d.npy'%load_epoch)
            prototype = torch.Tensor(np.load(prototype_path)).cuda()
        ...
        with tqdm.trange(start_epoch, n_epochs, desc='epochs') as tbar, \
            ...
            for epoch in tbar:
                ...
                for cur_it, batch in enumerate(train_loader):
                    ...
                    loss, tb_dict, disp_dict, prototype = self._train_it(batch,prototype)
                    it += 1
                ...
                prototype_path = os.path.join(self.ckpt_dir, 'proto')

                if not os.path.isdir(prototype_path):
                    os.mkdir(prototype_path)

                np.save(prototype_path + '/epoch_%d.npy'%trained_epoch,prototype.detach().cpu())
