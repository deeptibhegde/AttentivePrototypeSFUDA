...
def main():
    ...
    if args.ckpt is not None:
        ...
        proto_list = glob.glob(str(proto_dir / '*protoype_epoch_*.pth'))
        if len(proto_list) > 0:
            ckpt_list.sort(key=os.path.getmtime)
            prototype = np.load(proto_list[-1])

    model.train()  # before wrap to DistributedDataParallel to support fixed some parameters
    if dist_train:
        model = nn.parallel.DistributedDataParallel(model, device_ids=[cfg.LOCAL_RANK % torch.cuda.device_count()])
        prototype = nn.parallel.DistributedDataParallel(prototype, device_ids=[cfg.LOCAL_RANK % torch.cuda.device_count()])
    logger.info(model)
