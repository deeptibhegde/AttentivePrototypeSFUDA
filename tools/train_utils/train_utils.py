...
def train_one_epoch(model, optimizer, train_loader, model_func,proto_dir,prototype, lr_scheduler, accumulated_iter, optim_cfg,
...
def train_model(model, optimizer, train_loader, target_loader, model_func, lr_scheduler, optim_cfg,
                start_epoch, total_epochs, start_iter, rank, tb_log, ckpt_save_dir, ps_label_dir,prototype,proto_dir,
    ...
    train_att_proto = True if cfg.get('ATTPROTO_TRAIN', None) else False
    if train_att_proto:
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            if train_att_proto:
                if (cur_epoch in cfg.ATTPROTO_TRAIN.UPDATE_PSEUDO_LABEL) or \
                        ((cur_epoch % cfg.ATTPROTO_TRAIN.UPDATE_PSEUDO_LABEL_INTERVAL == 0)
                        and cur_epoch != 0):
                    train_loader.dataset.eval()
                    self_training_utils.save_pseudo_label_epoch(
                        model, train_loader, rank,
            ...
            accumulated_iter = train_one_epoch(
                model, optimizer, train_loader, model_func, proto_dir,prototype,
