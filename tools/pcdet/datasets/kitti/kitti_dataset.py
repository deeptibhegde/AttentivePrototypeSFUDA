...
class KittiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        split_dir = self.root_path / 'ImageSets' / (self.split + '_f.txt')
    ...
    def get_label(self, idx):
        label_file = self.root_split_path / 'label_2_waymo_ped_1' / ('%s.txt' % idx)
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    train_filename = save_path / ('infos_waymo_ped/kitti_infos_%s.pkl' % train_split)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        ROOT_DIR = Path('/data/datasets')#(Path(__file__).resolve().parent / '../../../').resolve()
        create_kitti_infos(
            dataset_cfg=dataset_cfg,
            class_names=['Car', 'Pedestrian', 'Cyclist'],
            data_path=ROOT_DIR /'kitti_all',
            save_path=ROOT_DIR /'kitti_all'
        )
