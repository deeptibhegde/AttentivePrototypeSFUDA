...
def check_already_exsit_pseudo_label(ps_label_dir, start_epoch):
    ...
    if train_att_proto:
        ...
        if start_epoch == 0 and cfg.ATTPROTO_TRAIN.get('INIT_PS', None):
            if  os.path.exists(cfg.ATTPROTO_TRAIN.INIT_PS):

                init_ps_label = pkl.load(open(cfg.ATTPROTO_TRAIN.INIT_PS, 'rb'))
                ...
                return cfg.ATTPROTO_TRAIN.INIT_PS
...
def save_pseudo_label_epoch(model, val_loader, rank, leave_pbar, ps_label_dir, cur_epoch):
    ...
    train_att_proto = True if cfg.get('ATTPROTO_TRAIN', None) else False
    if not train_att_proto:
        if cfg.SELF_TRAIN.get('DSNORM', None):
            model.apply(set_ds_target)
    else:
        if cfg.ATTPROTO_TRAIN.get('DSNORM', None):
            model.apply(set_ds_target)
    model.eval()
...
def save_pseudo_label_batch(input_dict,
    ...
    train_att_proto = True if cfg.get('ATTPROTO_TRAIN', None) else False
    if train_att_proto:
        cfg.SELF_TRAIN = cfg.ATTPROTO_TRAIN
