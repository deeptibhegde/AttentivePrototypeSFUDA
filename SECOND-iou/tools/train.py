...
def parse_config():
    ...
    parser.add_argument('--first_it', action='store_true', default=False, help='')
...
def main():
    ...
    # proto_dir = str(ckpt_dir)[:-4] + 'proto'
    ...
    if args.pretrained_model is not None:
        model.load_params_from_file(filename=args.pretrained_model, to_cpu=dist,first_it=args.first_it, logger=logger)
    ...
    if args.ckpt is not None:
        ...
        proto_list = glob.glob(str(proto_dir / '*prototype_*.npy'))
        if len(ckpt_list) > 0:
            ckpt_list.sort(key=os.path.getmtime)
            proto_list.sort(key=os.path.getmtime)
            it, start_epoch = model.load_params_with_optimizer(
                ckpt_list[-1], to_cpu=dist, optimizer=optimizer, logger=logger
            )
            last_epoch = start_epoch + 1
        if len(proto_list) > 0:
            prototype = torch.Tensor(np.load(proto_list[-1])).cuda()
